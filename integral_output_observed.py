...
class Integral_Output_Observed(Kern): #todo do I need to inherit from Stationary
    ...
    def dk_dl(self, t, tprime, s, sprime, lengthscale): #derivative of the kernel wrt lengthscale
        l = lengthscale * np.sqrt(2)    #TODO WHAT IS THIS FOR IS THIS MAKING THE LENGTHSCALE THE VALID ONE FOR THIS LATENT KERNEL (rbf)
...
#this function isn't used as we never need to find the covariance between the observed and latent functions
        ...
        if X2 is None: #X vs X
            ...
            for i,x in enumerate(X):
                for j,x2 in enumerate(X2):
                    for il,l in enumerate(self.lengthscale):
                        idx = il*2
                        #This is the only calculation that's changed from the standard multidimensional integral kernel (and the equivalent for Kdiag)
                        K_xx[i,j] *= self.k_xx(x[idx],x2[idx],x[idx+1],x2[idx+1],l)
            return K_xx * self.variances[0]
